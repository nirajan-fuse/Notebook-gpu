...
# the studio user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ]; then
    ...
    # Refit the studio user to the desired user (NB_USER)
    if id studio &> /dev/null; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" studio 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the studio user:"
            _log "- username: studio       -> ${NB_USER}"
            _log "- home dir: /home/studio -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the studio user nor '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the studio home directory to the desired user's home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "studio" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/studio to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            # shellcheck disable=SC2086
            if cp ${CP_OPTS:--a} /home/studio/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/studio to /home/${NB_USER}!"
                _log "Attempting to symlink /home/studio to /home/${NB_USER}..."
                if ln -s /home/studio "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/studio to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/studio/"* ]]; then
...
# user we started as.
    ...
    studio_UID="$(id -u studio 2>/dev/null)"  # The default UID for the studio user
    studio_GID="$(id -g studio 2>/dev/null)"  # The default GID for the studio user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old studio user to nayvoj ($(id -u studio):$(id -g studio))"
            ...
            sed --expression="s/^studio:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/studio:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "studio" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/studio. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/studio ]]; then
        _log "WARNING: no write access to /home/studio. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
